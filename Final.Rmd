...
I wanted to see which divisions were the strongest and which were the weakest. To do this I made this table by grouping by division and taking the average squat, bench, deadlift and total for each division. 
...
Then I took that data and plotted it into four bar graphs for each lift and the total.
...
```{r}
suppressMessages(lifters_by_equipment_pivoted%>%
  ggplot(aes(x=lift_type, y=avg_weight_kg, fill = equipment))+
  geom_bar(stat = "identity", position = "dodge"))
```

This graphs shows to no surprise that out of the three raw lifters are the weakest.What is interesting however is that it seems wraps is stronger than single-ply. I did not expect this to be the case but I think this is contributed to sample size. Wraps is a relatively new way of lifting in the IPF so it hasn't been around nearly as long as single-ply and wraps. Because of this and the fact that the more experienced lifters are probably eager to try a new lifting style I believe this is the reason why the graph portrays wraps to be the strongest way to lift. I predict that as time goes on and the sample size increases single-ply will eventually overtake wraps for strongest lifting style.
